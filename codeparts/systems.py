...
class system():
    ...
    def edit_settings():
        while True:
            ...
            proxyscraper = requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all')
            menu_choices = [
                Separator(),
                f'RLimits to skip an acc: {max_rlimits}',
                f'Wait if there is a limit (seconds): {rlimit_wait}',
